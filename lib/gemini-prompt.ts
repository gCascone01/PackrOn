...
export function buildTripPrompt(payload: GenerateTripPayload): string {
  ...
  const impossibleTripDetectionRoadIt =
    ...
    + "- Solo se, dopo aver interpretato l'input al meglio, l'origine o la destinazione risulta chiaramente non essere un luogo reale (stringhe senza senso, luoghi inventati, input vuoto o concetti non geografici): imposta \"impossible_trip\" a true, imposta \"error_code\" a \"invalid_origin\" o \"invalid_destination\" a seconda del caso, e spiega quale località è non valida in \"reason\" (es. \"Origine 'Xyz' non trovata\" o \"Destinazione 'Xyz' non trovata\"). Compila i restanti campi obbligatori con valori minimi e usa un array \"days\" vuoto. Non segnalare mai come impossibile un viaggio per un piccolo errore di ortografia.\n"
    + "- Rifiuta un viaggio per motivi geografici solo quando richiede davvero un volo: continenti senza né collegamento terrestre guidabile né collegamenti via traghetto (es. Europa–America attraverso l'Atlantico, Europa–Australia). Imposta \"impossible_trip\" a true, imposta \"error_code\" a \"impossible\" e spiega il motivo in \"reason\" (es. intercontinentale senza collegamenti terrestri o via traghetto, richiede volo). Compila i restanti campi obbligatori con valori minimi e usa un array \"days\" vuoto.\n"
    + "- I mari con collegamenti via traghetto sono normale territorio da road trip: il Mediterraneo è attraversabile (es. traghetti Italia–Grecia, Italia–Tunisia) e lo si può anche aggirare via Balcani e Turchia — es. Milano–Il Cairo è un road trip valido. Pianifica tali percorsi via terra e/o con tratte in traghetto indicate esplicitamente nell'itinerario, più avvertenze su frontiere/visti nelle descrizioni delle tappe pertinenti. Non definire mai il Mediterraneo inattraversabile e NON imporre alcun limite di distanza: i road trip lunghi sono validi.\n"
    + "- Non rifiutare mai un viaggio solo perché la durata è generosa rispetto alla distanza (es. 30 giorni per ~2500 km): i giorni extra significano deviazioni panoramiche, giorni di riposo ed esplorazione più approfondita. Il ritmo è un massimo giornaliero, non una quota da riempire — pianifica giornate di guida più brevi e più tempo per tappa.\n"
    + "- Complessità di frontiere, visti, permessi per il veicolo o geopolitica non è mai un motivo per rifiutare: pianifica sempre il percorso e segnala requisiti di attraversamento e avvertenze pratiche nelle descrizioni delle tappe pertinenti. Allo stesso modo, 'distanza eccessiva' o 'parametri tipici' non sono mai motivi di rifiuto — non imponi alcun limite di distanza."
